...
simsignal_t LteRealisticChannelModel::rsrp_ = registerSignal("rsrp");
...
std::vector<double> LteRealisticChannelModel::getRSRP(LteAirFrame *frame, UserControlInfo* lteInfo)
{
   ...
   for (unsigned int i = 0; i < numBands_; i++)
   {
       ...
       sumRsrp += rsrpVector[i];
       ++usedRBs;
   }
   //============ END PATH LOSS + SHADOWING + FADING ===============

   // EV << "Before Emit RSRP" << collectSinrStatistics_ << " " << lteInfo->getFrameType() << " " << usedRBs << " " << dir << endl;

   // emit SINR statistic
   if (collectSinrStatistics_ && (lteInfo->getFrameType() == BROADCASTPKT) && usedRBs > 0 && dir == DL)
   {
       // we are on the BS, so we need to retrieve the channel model of the sender
       // XXX I know, there might be a faster way...
       LteChannelModel* ueChannelModel = check_and_cast<LtePhyUe*>(getPhyByMacNodeId(ueId))->getChannelModel(lteInfo->getCarrierFrequency());
       ...
       // we are on the UE
       ueChannelModel->emit(rsrp_, sumRsrp / usedRBs);
   ...
   while(it!=et)
   {
       ...
       else // error computation. We need to check the slot occupation of the previous TTI
       {
           for(unsigned int i=0;i<numBands;i++)
           {
               ...
               // compute the number of occupied slot (unnecessary)
               //temp = (*it)->mac->getDlPrevBandStatus(i); // Causes a bug
               //if(temp!=0)
               (*interference)[i] += dBmToLinear(txPwr-att); //(dBm-dB)=dBm
