...
void LtePhyUe::initialize(int stage)
{
    ...
    if (stage == inet::INITSTAGE_LOCAL)
    {
        ...
        neighborTop1SINR_ = registerSignal("neighborTop1SINR");
        neighborTop1Distance_ = registerSignal("neighborTop1Distance");
        neighborTop2RSRP_ = registerSignal("neighborTop2RSRP");
        neighborTop2SINR_ = registerSignal("neighborTop2SINR");
...
//            WATCH(rxAmount_);
    ...
    else if (stage == inet::INITSTAGE_PHYSICAL_LAYER)
    {
        ...
        if (dynamicCellAssociation_)
        {
            ...
            updateHysteresisTh(currentMasterRssi_);
...
void LtePhyUe::checkEnbs()
{
    ...
    if (isNr_)
    {
        neighborRSRP_[0] = -999.0;
        neighborRSRP_[1] = -999.0;
        neighborSINR_[0] = -999.0;
        neighborSINR_[1] = -999.0;
    ...
    for (; it != enbList->end(); ++it)
    {
        ...
        // get RSRP from the BS
        double rsrp = 0;
        std::vector<double>::iterator it1;
        std::vector<double> rsrpV = primaryChannelModel_->getRSRP(frame, cInfo);
        for (it1 = rsrpV.begin(); it1 != rsrpV.end(); ++it1)
            rsrp += *it1;
        rsrp /= rsrpV.size();
        ...
        if (isNr_) {
            ...
            std::vector<double> sinrV = primaryChannelModel_->getSINR(frame, cInfo);
            for (it2 = sinrV.begin(); it2 != sinrV.end(); ++it2)
                sinr += *it2;
            sinr /= sinrV.size();
            ...
            if(cellId == masterId_) {
                ...
                if(rsrp > neighborRSRP_[0]) {
                    neighborRSRP_[1] = neighborRSRP_[0];
                    neighborRSRP_[0] = rsrp;
                } else if(rsrp > neighborRSRP_[1]) {
                    neighborRSRP_[1] = rsrp;
                }

                if(sinr > neighborSINR_[0]) {
                    neighborSINR_[1] = neighborSINR_[0];
                    neighborSINR_[0] = sinr;
                } else if(sinr > neighborSINR_[1]) {
                    neighborSINR_[1] = sinr;
                }

                if(distance < neighborDistance_[0]) {
                    neighborDistance_[1] = neighborDistance_[0];
                    neighborDistance_[0] = distance;
                } else if(distance < neighborDistance_[1]) {
                    neighborDistance_[1] = distance;
                }
            }
        }

        if (rsrp > candidateMasterRssi_)
        {
            candidateMasterId_ = cellId;
            candidateMasterRssi_ = rsrp;
        }
    }
...
void LtePhyUe::handoverHandler(LteAirFrame* frame, UserControlInfo* lteInfo)
{
    ...
    {
        // Broadcast message from my master enb
        sinr = das_->receiveBroadcast(frame, lteInfo);
    }
    else
    {
        ...
        for (it = sinrV.begin(); it != sinrV.end(); ++it)
            sinr += *it;
        sinr /= sinrV.size();
    }

    EV << "UE " << nodeId_ << " broadcast frame from " << lteInfo->getSourceId() << " with sinr: " << sinr << " at " << simTime() << endl;

    if (lteInfo->getSourceId() != masterId_ && sinr < minRssi_)
    {
    ...
    if (sinr > candidateMasterRssi_ + hysteresisTh_)
    {
        if (lteInfo->getSourceId() == masterId_)
        {
            // receiving even stronger broadcast from current master
            currentMasterRssi_ = sinr;
            candidateMasterId_ = masterId_;
            candidateMasterRssi_ = sinr;
        ...
        else
        {
            ...
            candidateMasterRssi_ = sinr;
            hysteresisTh_ = updateHysteresisTh(candidateMasterRssi_);
