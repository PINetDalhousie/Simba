...
def aggregate_across_basestations(
    ...
    for cellID in bsIds:
        ...
        if cellID != 1:
            ...
            # Iterate over each group and plot the distribution of rsrp
            for name, group in bsDF_group:
                plt.hist(group['servingRSRP:vector'])
                plt.title(f"rsrp distribution for cell {cellID}")
                plt.savefig(f"../data/prepared/{CURRENT_TIME}_rsrp_distribution_cell{cellID}_{name}.png")
                plt.close()
