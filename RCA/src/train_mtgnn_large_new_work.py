...
import shutil
shutil.copy('train_mtgnn_large.py', f'{log_path}/train_mtgnn_large.py')

# Parse arguments using argparse
import argparse
parser = argparse.ArgumentParser(description='Train MTGNN on large topology dataset')
parser.add_argument('--model', type=str, default='MTGNN', help='Model to use: MTGNN or TransformerMTGNN')
...
graph_data = pd.read_csv('../data/large_topology/20240312_181640_MTGNN.txt', sep=',', header=None)
...
def val(dataloader, model, loss_fn, summary_writer, epoch):
  ...
  with torch.no_grad():
    for X, y in dataloader:
      ...
      #val_precision = multiclass_precision(y, pred, num_classes=NUM_CLASSES, average=None)
      #val_recall = multiclass_recall(y, pred, num_classes=NUM_CLASSES, average=None)
