...
def aggregate_by_timedelta(df:pd.DataFrame, timedelta):
    ...
    for cellID in df['servingCell:vector'].unique():
        ...
        for UEid in bsDF['UEid:vector'].unique():
            ueDF = bsDF[bsDF['UEid:vector'] == UEid].copy(deep=True)
...
def aggregate_across_basestations(
    ...
    for cellID in df['servingCell:vector'].unique():
        ...
        # If save the dataframe with 'time_milli' equal to 6.0
        # if cellID == 5:
        #     bsDF[bsDF['time_milli'] == 6.0].to_csv(f"../data/2024-01-24/{CURRENT_TIME}_bs{cellID}_6.csv", index=False)

        # # List of unique 'time_milli' values
        # time_milli_list = bsDF['time_milli'].unique()
        # # Sort the list
        # time_milli_list.sort()
        # print(time_milli_list)

        # # Iterate over each unique 'time_milli' in bsDF
        # for time_milli in time_milli_list:
        #     print(time_milli)
        #     #if time_milli == 6.0:
        #     bsDF[bsDF['time_milli'] == time_milli].to_csv(f"../data/2024-01-24/{CURRENT_TIME}_bs{cellID}_{time_milli}_before.csv", index=False)

        #     # Make a deep copy of ueDF when bsDF['time_milli'] == time_milli
        #     ueDF = bsDF[bsDF['time_milli'] == time_milli].copy(deep=True)
            
        #     # Create a dict of lists to store the values for each column
        #     data = {}

        #     # Iterate over each column in ueDF
        #     for col in ueDF.columns:
        #         # Add the column name as a key in the dict and the column values as a list
        #         data[col] = list(ueDF[col].values)
        ...
        #     #print(f"mean {ueDF.mean()}")

        #     print(asd)

            
            # # Calculate the mean of ueDF
            # ueDF_group = ueDF.groupby(by=ueDF['time_milli'])
            # ueDF = ueDF_group.mean()

            # # If cellID is 5 and timstamp is 0 days 00:00:06.004000, save to csv
            # if cellID == 5 and time_milli == 6.0:
            #     ueDF.to_csv(f"../data/2024-01-24/{CURRENT_TIME}_bs{cellID}_{time_milli}_after.csv", index=False)

            # # Add the number of rows in ueDF_group as a column
            # ueDF['count'] = ueDF_group.size()
            # # Add ueDF to concat_DF
            # concat_DF.append(ueDF)
...
def prepare_data():
    ...
    parser.add_argument('--save_path', type=str, default="../data/medium-epr1/", 
                        help='The path to save the prepared data.')
    parser.add_argument('--data_path', type=str, default="../data/medium-epr1/medium-epr1.csv",
                        help='The path to the input data.')
    parser.add_argument('--gnb_map_txt', type=str, default="../data/medium-epr1/gnb-map.txt",
                        help='The path to the gnb-map.txt file.')
    parser.add_argument('--fault_description_txt', type=str, default="../data/medium-epr1/faults-description.txt",
    ...
    #epr_df.to_csv(f"../data/prepared/{CURRENT_TIME}_bs_5_after_ue.csv", index=True)
    #print(asd)
    ...
    # Set index as timestamp:vector column 
    epr_df['timestamp:vector'] = epr_df.index

    # create a column time_milli that is the timestamp:vector column converted to milliseconds
    epr_df['time_milli'] = epr_df['timestamp:vector'].dt.total_seconds().astype(float)
    ...
    # Remove timestamp:vector column
    #epr_df.drop(columns=['timestamp:vector'], inplace=True)

    print(f"after ue aggregation : {epr_df.info()}")
