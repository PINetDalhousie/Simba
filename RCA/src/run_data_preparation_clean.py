...
def aggregate_across_basestations(
    ...
    for cellID in df['servingCell:vector'].unique():
        ...
        for start,end,cells,fault_label in zip(fault_start,fault_end,servingCells,fault_labels):
            ...
            for cell in cells:
                bsDF.loc[(bsDF['servingCell:vector'] == cell) & (bsDF['timestamp:vector'] > start) & (bsDF['timestamp:vector'] < end), 'label'] = fault_label
...
def prepare_data():
    ...
    parser.add_argument('--save_path', type=str, default="../data/1h-faults-fixed/", 
                        help='The path to save the prepared data.')
    parser.add_argument('--data_path', type=str, default="../data/1h-faults-fixed/1h-faults-fixed.csv",
                        help='The path to the input data.')
    parser.add_argument('--gnb_map_txt', type=str, default="../data/1h-faults-fixed/gnb-map.txt",
                        help='The path to the gnb-map.txt file.')
    parser.add_argument('--fault_description_txt', type=str, default="../data/1h-faults-fixed/faults-description.txt",
    ...
    ############################
    # Aggregate data by seconds for base stations
    ############################
    ...
    ############################
    # Aggregate data from paired base stations
    ############################
    print(f"after aggregation: {len(epr_df)}")
    print(epr_df.info())
    #print(epr_df['servingCell:vector'].value_counts())
    epr_df = aggregate_paired_basestations(epr_df)
    # Reset index to integers
    epr_df.reset_index(drop=True, inplace=True)
    print(f"Dataframe info before saving :")
    print(epr_df.info())
    print(epr_df['servingCell:vector'].value_counts())
    ...
    ############################
    write_MTGNN_data(epr_df, args.save_path, keep_rows=KEEP_ROWS)
    
    ############################
    # Write data for FCN
    ############################
    write_FCN_data(epr_df, args.save_path, keep_rows=KEEP_ROWS)
