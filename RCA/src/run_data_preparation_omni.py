...
def aggregate_across_basestations(
    ...
    for cellID in bsIds:
        ...
        # Print value counts of time_mili
        #print(bsDF['time_milli'].value_counts())
        #print(asd)
        # Group by index
        bsDF_group = bsDF.groupby(by=bsDF['time_milli'])
        ...
        #     continue
        # else:
        #     # Plot the distribution of individual features using pandas plot
            


        # Calculate percentile features
        # Calculate 95th percentile of RSRP in the each group
        bsDF['rsrp_5'] = bsDF_group['servingRSRP:vector'].quantile(0.05)
        bsDF['rsrp_95'] = bsDF_group['servingRSRP:vector'].quantile(0.95)
        # Calculate 95th percentile of SINR in the each group
        bsDF['sinr_5'] = bsDF_group['servingSINR:vector'].quantile(0.05)
        bsDF['sinr_95'] = bsDF_group['servingSINR:vector'].quantile(0.95)
        # Calculate 5th percentile of RSRQ in the each group
        bsDF['rsrq_5'] = bsDF_group['servingRSRQ:vector'].quantile(0.05)
        bsDF['rsrq_95'] = bsDF_group['servingRSRQ:vector'].quantile(0.95)
        # Calculate distance percentiles
        bsDF['dist_5'] = bsDF_group['servingDistance:vector'].quantile(0.05)
        bsDF['dist_95'] = bsDF_group['servingDistance:vector'].quantile(0.95)
...
def prepare_data():
    ...
    parser.add_argument('--save_path', type=str, default="../data/7gnb-30-45ue/", 
                        help='The path to save the prepared data.')
    parser.add_argument('--data_path', type=str, default="../data/7gnb-30-45ue/7gnb-45ue.csv",
                        help='The path to the input data.')
    parser.add_argument('--fault_description_txt', type=str, default="../data/7gnb-30-45ue/fault-description-time-bin.txt",
    ...
    df['timestamp:vector'] = df.index
    # print(df['timestamp:vector'].head(20))
    # try:
    #     print(df['timestamp:vector'].dt.microseconds.head(20))
    # except:
    #     print(df['timestamp:vector'].dt.microseconds.astype(float).head(20))        

    # print(asd)
    df['time_milli'] = df['timestamp:vector'].dt.total_seconds().astype(float) + (df['timestamp:vector'].dt.microseconds / 1e3)
    # Reset index to integer values
    df.reset_index(drop=True, inplace=True)
    # Remove ueid column
